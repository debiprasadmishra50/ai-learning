"""
This script demonstrates a chain-of-prompt for answering customer queries about specific products using an LLM.
...
category_and_product_response_1 = get_openai_completion_from_messages(messages)
...
def generate_output_string(data_list):
    """
    Generate a formatted string containing detailed product information
    for each product or category in the provided data list.

    Args:
        data_list (list): List of dictionaries, each with either a 'products' key (list of product names)
                          or a 'category' key (category name).

    Returns:
        str: A string with JSON-formatted product details for each relevant product.
    """
    output_string = ""
    ...
    for data in data_list:
        try:
            # If the object contains a list of products, add their details
            if "products" in data:
            ...
            # If the object contains a category, add details for all products in that category
...
final_response = get_openai_completion_from_messages(messages)
